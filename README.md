...
A Google Photos-style personal photo library — built with React, Express, Azure Blob Storage, and PostgreSQL. Sign in with your Microsoft account and your photos are stored privately in your own Azure infrastructure.
...
- **Import from Google Photos** — uses the [Google Photos Picker API](https://developers.google.com/photos/picker) (OAuth 2.0 + `photospicker.mediaitems.readonly` scope)
- Trash — soft-delete with restore
- Archive / Hide — hide photos from the main library without deleting
- Dark mode
- Sign in with Microsoft / Azure Entra ID (Device Code Flow — works on any device)
...
2. Fill in:
   - **Name**: `aphoto-app` (or anything you like)
...
To enable the "Import from Google Photos" feature, set these additional env vars:
...
1. Enable the **Photos Picker API** (`photospicker.googleapis.com`)
2. Add scope `https://www.googleapis.com/auth/photospicker.mediaitems.readonly` to your OAuth consent screen
3. Add `https://<YOUR_API_URL>/api/google/callback` as an Authorized Redirect URI in your OAuth 2.0 credentials
...
```bash
# 1. Deactivate the backend — API returns 503
REVISION=$(az containerapp revision list --name my-photos-api \
  --resource-group my-photos-rg \
  --query "[?properties.active==\`true\`].name" -o tsv)
az containerapp revision deactivate \
  --name my-photos-api --resource-group my-photos-rg --revision "$REVISION"

# 2. Deploy a maintenance page to the frontend
mkdir -p /tmp/maintenance
cat > /tmp/maintenance/index.html << 'EOF'
<!DOCTYPE html><html><head><title>Temporarily Unavailable</title></head>
...
<div style="text-align:center"><h1>APhoto</h1><p>Temporarily offline. Back soon.</p></div>
</body></html>
EOF
SWA_TOKEN=$(az staticwebapp secrets list --name my-photos-frontend \
  --resource-group my-photos-rg --query "properties.apiKey" -o tsv)
npx @azure/static-web-apps-cli deploy /tmp/maintenance \
...
```bash
# 1. Re-activate the backend revision
az containerapp revision activate \
  --name my-photos-api --resource-group my-photos-rg --revision "$REVISION"

# 2. Rebuild and redeploy the real frontend
PORT=3000 BASE_PATH=/ \
  VITE_API_URL=https://<YOUR_CONTAINER_APP_URL> \
  pnpm --filter my-photos build

SWA_TOKEN=$(az staticwebapp secrets list --name my-photos-frontend \
  --resource-group my-photos-rg --query "properties.apiKey" -o tsv)
npx @azure/static-web-apps-cli deploy artifacts/my-photos/dist/public \
  --deployment-token "$SWA_TOKEN" --env production
```
...
Azure Container App           ← Express API (Node 22, Docker) — Central India
  ...
  ├── /api/photos/*           ← presign · register · list · search · favorite · trash · hide
  ├── /api/albums/*           ← create · list · add photos
  ├── /api/google/*           ← Picker API OAuth + background import
...
1. User clicks "Import from Google" → `POST /api/google/auth-url` returns a Google OAuth URL
2. User authorises → callback creates a Photos Picker session → redirects to `/albums?import_id=X`
3. Modal shows **Open Google Photos Picker** → user selects photos and clicks Done in Google's UI
4. Server polls the session until `mediaItemsSet=true`, then downloads each photo (with Auth Bearer token + `=d` / `=dv` suffix) and uploads to Blob Storage
...
| Layer | Control |
|---|---|
| Authentication | JWT (HS256, 7-day, `HttpOnly; Secure; SameSite=None`) in cookie — no tokens in JS memory or localStorage |
| Authorization | Every API route checks `req.user` (decoded from JWT); `requireAuth` middleware returns 401 otherwise |
| Secrets | All secrets (`JWT_SECRET`, `SESSION_SECRET`, DB password, Google credentials) stored as Container App secrets — never in env vars or code |
| Storage access | Keyless — Managed Identity + `Storage Blob Data Owner` role; no connection strings or account keys stored |
| Transport | HTTPS enforced end-to-end (Container App ingress + SWA both TLS-only); cookie `secure: true` in production |
| CORS | Configured per-origin on both the Express API and Azure Blob Storage |
| File uploads | 100 MB per-file limit enforced by multer; files stored under `userId/` prefix — users can only access their own blobs |
| OAuth state | CSRF protection via random UUID state parameter on both Microsoft and Google OAuth flows, stored in-memory with 10-minute TTL |
| Share tokens | 64-byte cryptographically random tokens (`randomBytes(32)`), stored in DB with expiry |
| SQL | All queries use Drizzle ORM parameterised queries — no raw SQL string interpolation |

> **Note**: `SESSION_SECRET` and `JWT_SECRET` *must* be set as Container App secrets. The app throws at startup if `SESSION_SECRET` is missing.
...
| Google Import | Google Photos Picker API (`photospicker.mediaitems.readonly`) |
